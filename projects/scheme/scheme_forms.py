...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        # BEGIN PROBLEM 10
        "*** YOUR CODE HERE ***"
        f_procedure = signature.first
        env.define(f_procedure, do_lambda_form(Pair(signature.rest, expressions.rest), env))
        return f_procedure
        # END PROBLEM 10
    else:
...
def do_and_form(expressions, env):
    ...
    while expressions is not nil:
        value0 = scheme_eval(expressions.first, env)
        if is_scheme_false(value0):
            return value0
...
def do_or_form(expressions, env):
    ...
    if expressions is nil: 
        return False
    while expressions is not nil: 
        value1 = scheme_eval(expressions.first, env)
        if is_scheme_true(value1):
            return value1
        expressions = expressions.rest
    return value1
    # END PROBLEM 12
