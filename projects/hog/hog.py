...
def boar_brawl(player_score, opponent_score):
    ...
    point = abs(one_digit - ten_digit) * 3
    if point > 1:
        score = point
    else:
        score = 1
...
def take_turn(num_rolls, player_score, opponent_score, dice=six_sided):
    ...
    # BEGIN PROBLEM 3
    "*** YOUR CODE HERE ***"
    if num_rolls == 0: # Bear Brawl rule
         return boar_brawl(player_score, opponent_score)
    return roll_dice(num_rolls, dice) # otherwise roll dice based on num_roll parameter
    # END PROBLEM 3
...
def sus_points(score):
    ...
    d_3 = 3
    d_4 = 4
    if num_factors(score) == d_3 or num_factors(score) == d_4: # or operator less lines of code 
        i = score 
        while not is_prime(i): # Ensure i moves forward, flip truthy value
             i += 1
        return i
    return score
    # END PROBLEM 4
...
def catch_up(score, opponent_score):
    ...
    if score < opponent_score:
        return 6  # Idea is to roll one more to catch up
    else:
        return 5
...
def is_always_roll(strategy, goal=GOAL):
    ...
    """
    # BEGIN PROBLEM 7
    reference = strategy(0, 0)  # Initial reference value
    for score in range(goal):  # Iterate over all possible scores
...
def max_scoring_num_rolls(dice=six_sided, times_called=1000):
    ...
    num_rolls = 1
    while i<= 10:
        ans = make_averaged(roll_dice, times_called)(i, dice)
        if ans > max:
            max = ans
            num_rolls = i
        i += 1
