...
class Ant(Insect):
    ...
    def __init__(self, health=1):
        ...
        self.damage_double = False # problem 12 
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            # BEGIN Problem 8b
            assert self.can_contain(place.ant) is True or place.ant.can_contain(self) is True, 'Too many ants in {0}'.format(place)
...
class ThrowerAnt(Ant): # inheritance of ant 
    ...
    def nearest_bee(self):
        ...
        og_place = self.place
        og_place_index = 0
        while not og_place.is_hive:
            if len(og_place.bees) > 0 and self.lower_bound <= og_place_index <= self.upper_bound:
                return random_bee(og_place.bees)
            else:
                og_place = og_place.entrance
            og_place_index += 1
        return None
        # END Problem 3 and 4
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        """
        # BEGIN Problem 5
        bee_list = list(self.place.bees)  # Copy bees before any changes
        total_damage = amount             # Initial reflective damage
        if self.health - amount <= 0:     # Check if this will kill the ant
            total_damage += self.damage   # Add extra damage if dying
        for bee in bee_list:              # Apply damage to all bees first
            bee.reduce_health(total_damage)
        super().reduce_health(amount)     # Then reduce own health and handle removal
        # END Problem 5
...
class WallAnt(Ant): 
    name = 'Wall'
    implemented = True # so you can use it in the game 
    food_cost = 4
...
class Water(Place):
    ...
    def add_insect(self, insect):
        ...
        # BEGIN Problem 10
        "*** YOUR CODE HERE ***"
        super().add_insect(insect) # inherit from place with super
        if not insect.is_waterproof: # use not statement 
...
class ScubaThrower(ThrowerAnt):
    ...
    # init the ThrowerAnt class with super and an init de function 
    def __init__(self):
        super().__init__()  
...
class QueenAnt(ThrowerAnt):
    ...
    def action(self, gamestate):
        ...
        "*** YOUR CODE HERE ***"
        super().action(gamestate)
        current_place = self.place.exit # allows for starting behind queeen 
        while current_place is not None:
            ant = current_place.ant
            if ant is not None:
                if not hasattr(ant, 'damage_double') or not ant.damage_double:
                    ant.double()
                # Handle contained ant if it’s a container
                if hasattr(ant, 'ant_contained') and ant.ant_contained is not None:
                    if not hasattr(ant.ant_contained, 'damage_double') or not ant.ant_contained.damage_double:
